...
namespace Jounce.Framework.ViewModel
{
    ...
    public class ViewModelRouter : IViewModelRouter, IFluentViewModelRouter
    {
        ...
        /// </summary>
        /// <param name="view">The view</param>
        /// <param name="specificViewModelName">a specific viewmodel name</param>
        /// <returns>The corresponding view model information</returns>
        private Lazy<IViewModel, IExportAsViewModelMetadata> GetViewModelInfoForView(string view, string specificViewModelName = null)
        {
            return (from r in _fluentRoutes
                    from vm in ViewModels
                    where r.ViewType.Equals(view)
                          && r.ViewModelType.Equals(vm.Metadata.ViewModelType) && (specificViewModelName ==null || specificViewModelName.ToLower() == r.ViewModelType.ToLower()) 
        ...
        public bool ActivateView(string viewName, IDictionary<string, object> parameters)
        {
            ...
            if (HasView(viewName))
            {
                ...
                string specificViewModelName = null;
                if (parameters.ContainsKey(Constants.SPECIFIC_VIEWMODEL_NAME))
                {
                    specificViewModelName = parameters[Constants.SPECIFIC_VIEWMODEL_NAME].ToString();
                }

                var viewModelInfo = GetViewModelInfoForView(viewName, specificViewModelName);
