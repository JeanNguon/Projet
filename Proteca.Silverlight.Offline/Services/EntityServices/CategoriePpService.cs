...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    public class CategoriePpService : IEntityService<CategoriePp>
    {
        ...
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            if (domainContext.CategoriePps.Count > 0)
            {
                Entities = new ObservableCollection<CategoriePp>(domainContext.CategoriePps.OrderBy(c => c.NumeroOrdre).ThenBy(c => c.Libelle));
            }
            completed(null);
        }
