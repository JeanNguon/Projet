...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    public class RefNiveauSensibilitePpService : IEntityService<RefNiveauSensibilitePp>
    {
        ...
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            if (domainContext.RefNiveauSensibilitePps.Count > 0)
            {
                Entities = new ObservableCollection<RefNiveauSensibilitePp>(domainContext.RefNiveauSensibilitePps.OrderBy(r => r.CleNiveauSensibilite));
            }
            completed(null);
        }
