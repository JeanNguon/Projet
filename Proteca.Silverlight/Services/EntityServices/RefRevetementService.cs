...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    /// 
    /// </summary>
    [Export(typeof(IEntityService<RefRevetement>))]
    public class RefRevetementService : IEntityService<RefRevetement>
    {
        #region Properties
        ...
        public ObservableCollection<RefRevetement> Entities { get; set; }

        public RefRevetement DetailEntity { get; set; }
        #endregion

        #region Constructor
        public RefRevetementService()
        {
        }
        #endregion
        ...
        public void Add(RefRevetement entity)
        {
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void Delete(RefRevetement entity)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntitySet es = domainContext.EntityContainer.GetEntitySet(entity.GetType());
            es.Remove(entity);
        ...
        public void SaveChanges(Action<Exception> completed)
        {
            ...
            if (domainContext.RefRevetements != null && domainContext.RefRevetements.HasChanges)
            {
                bool isValid = true;
                foreach (var obj in domainContext.RefRevetements.EntityContainer.GetChanges())
                {
        ...
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntityQuery<RefRevetement> query = domainContext.GetRefRevetementQuery();
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                ...
                {
                    // Affectation de l'entité "Non défini" à la position 0 dans la liste puis tri sur le libellé
                    ObservableCollection<RefRevetement> ListeRefRevetement = new ObservableCollection<RefRevetement>();
                    RefRevetement refrev = loadOp.Entities.Where(r => r.Code == RefEnum.NonDefini.CodeNonDefiniRevetement.GetStringValue()).FirstOrDefault();

                    if (refrev != null)
                    {
                        ListeRefRevetement.Add(refrev);
                    }

                    foreach (var refreve in loadOp.Entities.OrderBy(r => r.Libelle))
                    {
                        if (refreve.Code != RefEnum.NonDefini.CodeNonDefiniRevetement.GetStringValue())
                        {
                            ListeRefRevetement.Add(refreve);
                        }
                    }
                    Entities = ListeRefRevetement;
                }
        ...
        public void GetEntityByCle(int cle, Action<Exception> completed)
        {
            ...
            EntityQuery<RefRevetement> query = domainContext.GetRefRevetementByCleQuery(cle);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                ...
                else
                {
                    ...
                    {
                        Entities = new ObservableCollection<RefRevetement>(loadOp.Entities);
                    }
                }
        ...
        /// <param name="completed">callback fonction</param>
        public void FindEntities(List<Expression<Func<RefRevetement, bool>>> filtres, Action<Exception> completed)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            ...
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<RefRevetement> entities = null;
                ...
                else
                {
                    ...
                    Entities = new ObservableCollection<RefRevetement>(entities.ToList());
                }
