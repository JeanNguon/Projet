...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    /// 
    /// </summary>
    [Export(typeof(IEntityService<RefUsrPortee>))]
    public class RefUsrPorteeService : IEntityService<RefUsrPortee>
    {
        #region Properties
        ...
        public ObservableCollection<RefUsrPortee> Entities { get; set; }

        public RefUsrPortee DetailEntity { get; set; }
        #endregion

        #region Constructor
        public RefUsrPorteeService()
        {
            Entities = new ObservableCollection<RefUsrPortee>();
        }
        #endregion
        ...
        public void Add(RefUsrPortee entity)
        {
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void Delete(RefUsrPortee entity)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntitySet es = domainContext.EntityContainer.GetEntitySet(entity.GetType());
            es.Remove(entity);
        ...
        public void SaveChanges(Action<Exception> completed)
        {
            ...
            if (domainContext.RefUsrPortees.HasChanges)
            {
                bool isValid = true;
                foreach (var obj in domainContext.RefUsrPortees.EntityContainer.GetChanges())
                {
        ...
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntityQuery<RefUsrPortee> query = domainContext.GetRefUsrPorteeQuery().OrderBy(u => u.CodePortee);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<RefUsrPortee> RefUsrPortees = null;
                ...
                {
                    RefUsrPortees = loadOp.Entities;
                    Entities = new ObservableCollection<RefUsrPortee>(RefUsrPortees.ToList());
                }
        ...
        public void GetEntityByCle(int cle, Action<Exception> completed)
        {
            ...
            EntityQuery<RefUsrPortee> query = domainContext.GetRefUsrPorteeByCleQuery(cle);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                ...
                else
                {
                    ...
                    {
                        Entities = new ObservableCollection<RefUsrPortee>(loadOp.Entities);
                    }
                }
        ...
        /// <param name="completed">callback fonction</param>
        public void FindEntities(List<Expression<Func<RefUsrPortee, bool>>> filtres, Action<Exception> completed)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);

            EntityQuery<RefUsrPortee> query = domainContext.GetRefUsrPorteeQuery();
            ...
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<RefUsrPortee> entities = null;
                ...
                else
                {
                    ...
                    Entities = new ObservableCollection<RefUsrPortee>(entities.ToList());
                }
