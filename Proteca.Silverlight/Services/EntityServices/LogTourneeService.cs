...
namespace Proteca.Silverlight.Services
{
    ...
    /// 
    /// </summary>
    [Export(typeof(IEntityService<LogTournee>))]
    public class LogTourneeService : IEntityService<LogTournee>
    {
        #region Properties
        ...
        public ObservableCollection<LogTournee> Entities { get; set; }

        public LogTournee DetailEntity { get; set; }
        #endregion

        #region Constructor
        public LogTourneeService()
        {
            this.Entities = new ObservableCollection<LogTournee>();
        }
        #endregion
        ...
        public void Add(LogTournee entity)
        {
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void Delete(LogTournee entity)
        {
            Logger.Log(LogSeverity.Verbose, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntitySet es = domainContext.EntityContainer.GetEntitySet(entity.GetType());
            es.Remove(entity);
        ...
        public void Clear()
        {
            domainContext.LogTournees.Clear();
            this.Entities = new ObservableCollection<LogTournee>();
        }
        ...
        public void SaveChanges(Action<Exception> completed)
        {
            ...
            if (domainContext.LogTournees != null && domainContext.LogTournees.HasChanges)
            {
                bool isValid = true;
                foreach (var obj in domainContext.LogTournees.EntityContainer.GetChanges())
                {
        ...
        public void GetEntityByCle(int cle, Action<Exception> completed)
        {
            ...
            EntityQuery<LogTournee> query = domainContext.GetLogTourneeByCleQuery(cle);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                ...
                else
                {
                    ...
                    {
                        Entities = new ObservableCollection<LogTournee>(loadOp.Entities);
                    }
                }
        ...
        /// <param name="completed">callback fonction</param>
        public void FindEntities(List<Expression<Func<LogTournee, bool>>> filtres, Action<Exception> completed)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);

            EntityQuery<LogTournee> query = domainContext.GetLogTourneeQuery();
            ...
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<LogTournee> entities = null;
                ...
                else
                {
                    ...
                    Entities = new ObservableCollection<LogTournee>(entities.ToList());
                }
        ...
        public void GetLogTourneeByCleTournee(int cleTournee, Action<Exception, List<LogTournee>> completed)
        {
            EntityQuery<LogTournee> query = domainContext.GetLogTourneeByCleTourneeQuery(cleTournee);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                Exception error = null;
                List<LogTournee> logTournee = new List<LogTournee>();
                if (loadOp.HasError)
                {
                    error = loadOp.Error;
                    Logger.Log(LogSeverity.Error, GetType().FullName, error);
                ...
                {
                    logTournee = loadOp.Entities.ToList();
                }

                completed(error, logTournee);
            }, null);
        }
