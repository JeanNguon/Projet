...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    /// 
    /// </summary>
    [Export(typeof(IEntityService<PpEquipement>))]
    public class PpEquipementService : IEntityService<PpEquipement>
    {
        #region Properties
        ...
        public ObservableCollection<PpEquipement> Entities { get; set; }

        public PpEquipement DetailEntity { get; set; }
        #endregion

        #region Constructor
        public PpEquipementService()
        {
        }
        #endregion
        ...
        public void Add(PpEquipement entity)
        {
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void Delete(PpEquipement entity)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntitySet es = domainContext.EntityContainer.GetEntitySet(entity.GetType());
            es.Remove(entity);
        ...
        public void Clear()
        {
            domainContext.PpEquipements.Clear();
            this.Entities = new ObservableCollection<PpEquipement>();
        }
        ...
        public void SaveChanges(Action<Exception> completed)
        {
            ...
            if (domainContext.PpEquipements != null && domainContext.PpEquipements.HasChanges)
            {
                bool isValid = true;
                foreach (var obj in domainContext.PpEquipements.EntityContainer.GetChanges())
                {
        ...
        /// <param name="completed">callback fonction</param>
        public void GetEntitiesByClePortion(int cle, Action<Exception> completed)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntityQuery<PpEquipement> query = domainContext.GetPpEquipementNoTrackingByClePortionQuery(cle);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<PpEquipement> PpEquipements = null;
                ...
                {
                    PpEquipements = loadOp.Entities;
                    Entities = new ObservableCollection<PpEquipement>(PpEquipements.ToList());
                }
        ...
        public void GetEntityByCle(int cle, Action<Exception> completed)
        {
            ...
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                ...
                else
                {
                    ...
                    {
                        Entities = new ObservableCollection<PpEquipement>(loadOp.Entities);
                    }
                }
        ...
        /// <param name="completed">callback fonction</param>
        public void FindEntities(List<Expression<Func<PpEquipement, bool>>> filtres, Action<Exception> completed)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);

            EntityQuery<PpEquipement> query = domainContext.GetPpEquipementNoTrackingQuery();
            ...
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<PpEquipement> entities = null;
                ...
                else
                {
                    ...
                    Entities = new ObservableCollection<PpEquipement>(entities.ToList());
                }
