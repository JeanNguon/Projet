...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    public class TypeEquipementService : IEntityService<TypeEquipement>
    {
        ...
        public void Clear()
        {
            //this.domainContext.TypeEquipements.Clear();
        ...
        public void GetEntities(Action<Exception> completed)
        {
            ...
            if (Entities != null && Entities.Count > 0 && !Entities.Any(t => t.EntityState == EntityState.Detached))
            {
            ...
            {
                EntityQuery<TypeEquipement> query = domainContext.GetTypeEquipementQuery();

                domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
                {
                    // Declare error and result
                    Exception error = null;
                    IEnumerable<TypeEquipement> TypeEquipements = null;
                    ...
                    {
                        TypeEquipements = loadOp.Entities;
                        //if (Entities.Count != TypeEquipements.Count())
                        {
                            Entities = new ObservableCollection<TypeEquipement>(TypeEquipements.ToList().OrderBy(r => r.NumeroOrdre));
                        }
                    }
        ...
        public void GetEntityByCle(int cle, Action<Exception> completed)
        {
            ...
            EntityQuery<TypeEquipement> query = domainContext.GetTypeEquipementByCleQuery(cle);
            domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                ...
                else
                {
                    ...
                    {
                        Entities = new ObservableCollection<TypeEquipement>(loadOp.Entities);
                    }
                }
