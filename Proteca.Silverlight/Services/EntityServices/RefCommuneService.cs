...
namespace Proteca.Silverlight.Services.EntityServices
{
    ...
    /// 
    /// </summary>
    [Export(typeof(IEntityService<RefCommune>))]
    public class RefCommuneService : IEntityService<RefCommune>
    {
        #region Properties
        ...
        public ObservableCollection<RefCommune> Entities { get; set; }

        public RefCommune DetailEntity { get; set; }
        #endregion

        #region Constructor
        public RefCommuneService()
        {
            Entities = new ObservableCollection<RefCommune>();
        }
        #endregion
        ...
        public void Add(RefCommune entity)
        {
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void Delete(RefCommune entity)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);
            EntitySet es = domainContext.EntityContainer.GetEntitySet(entity.GetType());
            es.Remove(entity);
        ...
        public void SaveChanges(Action<Exception> completed)
        {
            ...
            if (domainContext.RefCommunes != null && domainContext.RefCommunes.HasChanges)
            {
                bool isValid = true;
                foreach (var obj in domainContext.RefCommunes.EntityContainer.GetChanges())
                {
        ...
        public void GetEntities(Action<Exception> completed)
        {
            ...
            {
                //Entities = new ObservableCollection<RefCommune>(domainContext.RefCommunes);
                completed(null);
            }
            else
            {
                EntityQuery<RefCommune> query = domainContext.GetRefCommuneNoTrackingQuery();
                domainContext.Load(query, LoadBehavior.MergeIntoCurrent, loadOp =>
                {
                    // Declare error and result
                    Exception error = null;
                    IEnumerable<RefCommune> RefCommunes = null;
                    ...
                    {
                        RefCommunes = loadOp.Entities;
                        Entities = new ObservableCollection<RefCommune>(RefCommunes);
                    }
        ...
        public void GetEntityByCle(int cle, Action<Exception> completed)
        {
            ...
            EntityQuery<RefCommune> query = domainContext.GetRefCommuneByCleQuery(cle);
            domainContext.Load(query, loadOp =>
            {
                ...
                else
                {
                    ...
                    {
                        Entities = new ObservableCollection<RefCommune>(loadOp.Entities);
                    }
                }
        ...
        /// <param name="completed">callback fonction</param>
        public void FindEntities(List<Expression<Func<RefCommune, bool>>> filtres, Action<Exception> completed)
        {
            Logger.Log(LogSeverity.Information, GetType().FullName, MethodBase.GetCurrentMethod().Name);

            EntityQuery<RefCommune> query = domainContext.GetRefCommuneQuery();
            ...
            domainContext.Load(query, loadOp =>
            {
                // Declare error and result
                Exception error = null;
                IEnumerable<RefCommune> entities = null;
                ...
                else
                {
                    ...
                    Entities = new ObservableCollection<RefCommune>(entities.ToList());
                }
